...
public class CollectionUtils {
     ...
     * 获取集合的唯一值
     */
    public static <T> T getUniqueValue(Collection<T> coll) {
    ...
    public static final <T> List<T> union(Collection<T> first, Collection<T> second) {
        List<T> unionList = (first == null ? CollectionUtils.emptyList() : new ArrayList<>(first));
        unionList.addAll(Optional.ofNullable(second).orElse(CollectionUtils.emptyList()));
    ...
    public static final <T> List<T> intersection(Collection<T> first, Collection<T> second) {
        if (first == null
                || second == null) {
            return (List<T>) Optional.ofNullable(first).orElse(Optional.ofNullable(second).orElse(CollectionUtils.emptyList()));
        }
    ...
    public static final boolean containsIgnoreCase(Map<String, Object> map, String key) {
        if (map == null || map.isEmpty()) {
            return false;
        }
    ...
    public static final <T> void sort(List<T> list, Comparator<T> comparator) {
        ...
        while(sortedList.size() < size) {
            ...
            if (!minFound) {
                throw new BusinessException("Infinitive loop occur while sorting, please check code.");
            }
        }
